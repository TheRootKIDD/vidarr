...
**Source:** "The big-DC MoE LLM design" by **Bjarke Hammersholt Roune** (19-point design note), v1 and v2 in `docs/source/` (with extracted text `v1.txt`, `v2.txt`); v2 is current. Roune's linked in-depth document, "Designing AI Chip Hardware and Software" (2026, 154 pp., a public Google Doc), is beside it as `Designing_AI_chip_software_and_hardware.{pdf,txt}`; its summary against P1–P19 is in `docs/05`.
**Tooling:** Claude Code. `CLAUDE.md` holds the session rules.
**Repositories:** public mirror at [github.com/TheRootKIDD/vidarr](https://github.com/TheRootKIDD/vidarr); development on [gitlab.com/rootkidd/vidarr](https://gitlab.com/rootkidd/vidarr). Both receive every commit.
