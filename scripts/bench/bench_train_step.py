...
class RecurrentStub(nn.Module):
    """Placeholder for the recurrent-MoE config.

    Deliberately not implemented: it requires the shared middle block, router,
    depth embedding and ragged-depth semantics of `docs/01 §4`, which is the
    Phase-1 reference implementation. Measuring a hand-rolled approximation here
    would produce a number that does not describe the model that will be trained.
    """

    def __init__(self, s: Shape) -> None:
        super().__init__()
        raise NotImplementedError("recurrent config: Phase 1, model/ — see docstring")
...
def run_steps(
    ...
    def step() -> None:
        ...
        with torch.autocast("cuda", dtype=torch.bfloat16):
            ...
            loss = F.cross_entropy(logits[:, :-1].reshape(-1, s.vocab), idx[:, 1:].reshape(-1))
        loss.backward()
        opt.step()
...
def _ddp_worker(
    rank: int, world: int, s: Shape, mb: int, steps: int, warmup: int, out: dict
) -> None:
    ...
    out[rank] = run_steps(rank, world, s, mb, steps, warmup, ddp=True)
    dist.barrier()
...
def main() -> None:
    ...
    if args.ddp:
        ...
        mp.spawn(
            _ddp_worker,
            args=(world, s, args.micro_batch, args.steps, args.warmup, out),
            nprocs=world,
            join=True,
        )
        ...
        world = 1
        per_gpu = [run_steps(0, 1, s, args.micro_batch, args.steps, args.warmup, ddp=False)]
    ...
    payload = {
        ...
        "recurrent_config": "stub — NotImplemented; Phase 1 (see RecurrentStub docstring)",
    }
