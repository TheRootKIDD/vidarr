...
`model/` is implemented and the ladder is running at `screen` over the ADR-014 corpus: `100-l0-screen` gives $L_{ref}$ = 3.336 nats; **106–108** (L1, L2, L3) are done; **109** (L5) was killed by a power outage at step 170 and stays as a partial record; **110** (L5) is done at 3.3328 nats (matches L0, trails L1 by 4 % at a fifth of the params — H6 undecided until `L5-ne128`); **111** (L5d) is done at 3.3477 nats (+0.45 % vs L5 against H15a's 2 % allowance); **112** L5-ne128 is done at 3.2232 nats (within 0.57 % of L1 at 68 % of its params; H6 silent at one seed); **113** L3-full is done (3.2342; subsampling was worth ≈ 1 point of acceptance, I27 #3 closed); **114** L5 at $r$ = 4 is done (3.3567, +0.72 % vs $r$ = 8 for half the middle-block FLOPs — H7's threshold met by a fixed schedule, so L6/L6c score against the fixed-$r$ curve); **115** L5 at $r$ = 12 is done (3.3370, +0.13 % — the depth curve is flat past 8); **queue 4 is finished and nothing is running**. **Machine shut down 2026-09-14 for the GPU 0 ↔ GPU 3 slot swap; on restart follow the pick-up checklist in `docs/04 §Session log` 2026-09-14 (env capture `027`, soak `028`, track cards by UUID) before queueing anything**; after that the ladder's own next step is the `small` programme (start with the L0 pair, which supplies σ), or the `screen` variants L5e/L6c after their ADRs — ids 101–105 are burnt, next free id is 116. **The `small` programme is under way (2.5 B tokens, 2 seeds; `docs/04 §Session log` from 2026-09-15 is the record):** $L_{ref}$(small) = 2.9958 (`116`/`117`), L1 pair 2.8839 (`130`/`119`), L2 pair 2.8851 (`120`/`121`, **H2 quality clause supported**), L3 pair 2.9682 (`122`/`131`, **H13 weakened on both clauses** — +2.88 % on the main head, acceptance 42.5 / 25.6 / 17.6 %; L3b is the only route left), L5 pair 3.0748 (`132`/`133`, **+2.64 % vs dense** at two seeds — the `screen` tie was a 1 B-token effect; no H6 verdict, the matched arm L5-ne128 at `small` is what H6 waits on), L5d pair 3.0896 (`134`/`135`, **+0.48 % vs the L5 pair — H15a supported at two seeds**, same cost as at `screen`; the 8 k-context eval is still owed), σ pooled over six pairs = 0.0031 nats (2σ 0.0061, 4σ 0.0123) — the `small` pair programme is complete; L5-ne128 seed 0 2.9588 (`138`, **+2.60 % vs the L1 pair, 12× the "matches" band — provisional *weakens* H6** with I29's 68 %-params caveat; beats dense by 1.24 %; the 128-expert router partitions experts by depth where 8 did not); `136` L5-noej `screen` (E-Q2) is silent (+0.20 % vs `110`) and its routing probe shows no depth-specific expert partitioning with or without $e_j$; `137` L7b `screen` (H8, $L_e$ = 2) is +0.57 % vs `110`, 0.07 points over the 0.5 % allowance — inconclusive at one seed, a `small` pair would decide; ids `118` and `123`–`129` are burnt; **Paused 2026-09-25 16:30 on the user's request** (`139` killed at step 850 and burnt; nothing running); on pick-up follow `docs/04 §Session log` 2026-09-25 (16:30): `nohup /mnt/nvme/queue_small_6.sh` = `142` L5-ne128 s1 rerun (H6 pair with `138`), `143`/`144` L7b pair (H8), ≈ 3.1 days, then re-arm the hourly loop; next ladder id 145, next rig id 031. Every `results.md` states its H and its verdict against $L_{ref}$ under ADR-013's bands.
