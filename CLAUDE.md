...
`model/` is implemented and the ladder is running at `screen` over the ADR-014 corpus: `100-l0-screen` gives $L_{ref}$ = 3.336 nats; **106–108** (L1, L2, L3) are done; **109** (L5) was killed by a power outage at step 170 and stays as a partial record; **110** (L5) and **111** (L5d) run from `/mnt/nvme/queue_screen_3.sh` — ids 101–105 are burnt. Every `results.md` states its H and its verdict against $L_{ref}$ under ADR-013's bands.
