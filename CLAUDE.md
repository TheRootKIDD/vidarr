...
`model/` is implemented and the ladder is running at `screen` over the ADR-014 corpus: `100-l0-screen` gives $L_{ref}$ = 3.336 nats; **106–108** (L1, L2, L3) are done; **109** (L5) was killed by a power outage at step 170 and stays as a partial record; **110** (L5) is done at 3.3328 nats (matches L0, trails L1 by 4 % at a fifth of the params — H6 undecided until `L5-ne128`); **111** (L5d) is done at 3.3477 nats (+0.45 % vs L5 against H15a's 2 % allowance); **112** L5-ne128 is done at 3.2232 nats (within 0.57 % of L1 at 68 % of its params; H6 silent at one seed); queue 4 (`/mnt/nvme/queue_screen_4.sh`) is running **113** L3-full, then **114/115** the L5-r sweep — ids 101–105 are burnt, next free id is 116. Every `results.md` states its H and its verdict against $L_{ref}$ under ADR-013's bands.
