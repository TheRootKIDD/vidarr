...
- **I27** **H13's acceptance metric measures the wrong quantity, and three experiments follow from fixing it.** ADR-013 states H13's threshold as *"greedy acceptance of heads 2/3/4 **against the main head** on held-out ≥ 70 / 55 / 45 %"*; ADR-012 repeats it. `model/mtp.py` logs `logits.argmax(-1) == target` against the **ground-truth** token — top-1 accuracy, not acceptance. A draft is accepted when it *agrees with the verifier*, not when it is *right*, so the two differ and acceptance is the larger. Found 2026-09-11 while asking what `108`'s result implies. CLAUDE.md: the doc wins.

  **Re-measured from `108`'s checkpoint** (`scripts/analysis/probe_mtp_acceptance.py`, offline, CPU): acceptance **38.5 / 21.6 / 15.0 %** for heads 2/3/4 against the logged **21.1 / 12.0 / 8.4 %**; main head's own top-1 is 40.0 %. **H13 still misses** — 38.5 vs a 70 % target is not a near miss — so `108`'s conclusion and ADR-030 both stand. But the margin is about half what was reported, and the failure is a different *kind*: head $j$ must anticipate a head 1 that sees $j-1$ more tokens of context, so acceptance measures how well it closes an information gap. **Not a ceiling argument**: head $j$ need only guess what head 1 will *say*, so the ceiling is 100 %, not head 1's 40 %.

  **Fix (not an experiment):** `model/mtp.py` should log both, named so they cannot be confused, and `01 §7` should state the definition. Do it between rungs, never mid-rung — the `screen` sweep must stay on one git hash (the ADR-030 pattern). *Done 2026-09-11 (night), between `109` and `110`:* the trainer now logs `mtp_top1_h{j}` and `mtp_accept_h{j}`, `01 §7` states the definition, and the unit test checks both against a manual computation. L5/L5d have no MTP heads, so the hash change does not touch their numbers.

  **Three experiments this raises**, in order of information per GPU-hour:
  1. **Expected accepted run length, offline.** H13's thresholds are *per-head marginals*, but what determines speculative-decode speedup is the expected number of tokens accepted per verify pass, and acceptance in a run is *sequential* — the window stops at the first rejection — so the joint matters, not the marginals. With these marginals and an independence assumption it is ≈ 1.48 tokens/pass; the true joint is lower, because the same hard positions defeat every head. **This is arguably the number H13 should have been written in.** Extends the existing probe; costs minutes; no GPU.
...
- **I28** *(direction, not scheduled — raised by the user 2026-09-11 for the post-ladder brainstorm)* **Composing independently trained small models into one larger one.** Train a spine once, freeze it, and let domain "experts" be trained separately on consumer hardware and inserted as they are developed — one composed model, or a network of model modules. This is P18 taken as the *training method* rather than a serving feature, with P17's memory table as the second composable object. Precedents: FlexOlmo / FlexMoRE (frozen public base, experts trained apart, domain-informed router, no joint training; `05 §P18`), Branch-Train-MiX. What the ladder bears on it: the recurrent spine (L5, 77 M vs 271 M layered) is what makes per-module training feasible on one 12 GB card; the routing transient in `108` (35 dead of 96, recovered by step 100) is the late-insertion problem in miniature, and whether the aux-loss-free controller admits a late expert is a `screen`-sized question; L7c/S11 decide whether contributor modules must share one width (P4); L8 decides whether inserted *memory* beats inserted *experts* as the composition mechanism. The control the literature omits and this project would require: composed-from-frozen-spine vs jointly trained at matched params and FLOPs on held-out loss — the cost of freezing is the number. Blockers before scheduling: a domain-corpus ADR beside ADR-014 (FineWeb-Edu has no domain split), and a metric for request-level "models talking" composition, which held-out loss does not score. Bears on H6, H17, H18, P4.
...
**Correction the same evening (I27): `108`'s acceptance figures measure the wrong quantity.** The
trainer logs top-1 accuracy against ground truth; ADR-013 and ADR-012 both define the threshold as
acceptance against head 1. Re-measured offline from the checkpoint: **38.5 / 21.6 / 15.0 %**, not
21.5 / 12.6 / 8.9 %. **H13 still misses and every conclusion above stands**, but the margin is about
half what was reported. Three experiments follow, two of them offline and cheap — see I27.

**Thermal rule widened, with the user's agreement.** `109` tripped the watch's "4 of 6 records
flagged" rule at step 10 and held it: GPU 0 brushes its limit continuously under L5's heavier
