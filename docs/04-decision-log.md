...
- **I16** `012-tiers-nvme` measured the cold tier at queue depth 1 (one synchronous reader). The drive is rated ~10⁶ random 4 KiB IOPS at high queue depth, so concurrent cold fetches from many in-flight sequences would land well above the 14 k blocks/s recorded. Before an S6 scenario models concurrent cold hits, add a `--queue-depth` option to `bench_tiers` (threads or `io_uring`) and rerun; until then `local_3060` carries the QD 1 numbers and the simulated cold tier is pessimistic.
