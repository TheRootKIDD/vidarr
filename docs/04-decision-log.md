...
- **ADR-022** — Local range: static window $W$ vs the dynamic in-flight region (all tokens not yet complete, which under pipelined segments can exceed $W$); and whether local attention also reaches the previous segment's per-iteration caches (Transformer-XL style). Default until decided: static $W$, previous segment not visible locally.
...
- **Q11** *(new, v2 P7)* "See all the prior layer KV caches with local attention": at iteration $j$, does local attention see neighbours only at iteration $j$ (standard layered semantics), or at every iteration $j' \le j$?
- **Q12** *(new, v2 P7)* The "less effective" super-parallel prefill mode: local attention only, global attention over a coarser summary, or pipelining segments so that segment $k+1$ at iteration $j$ sees segment $k$ at iteration $j$? And how deep may the in-flight region grow (ADR-022)?
...
| Q11–12 | author's intent | which local semantics / prefill mode work best at our scale | L5d variants (same-iteration vs all-prior-iteration local range; previous segment visible or not) | 2 extra `screen` ≈ 16 h | Transformer-XL, Block-Recurrent Transformers |
