...
$m$ heads on $p_t^{out}$ ($c_t^{out}$ single-stream): head 1 predicts $x_{t+1}$ (main loss); heads $2..m$ predict $x_{t+2..t+m}$ as **independent heads** — RMSNorm → linear $d \to d$ adapter → tied output embedding — with each position contributing to exactly one auxiliary head per step (`mtp.subsample`, ADR-012; sequential modules are the optional L3b). Loss weights $\lambda_2 \ge \lambda_3 \ge \lambda_4$. Decode: draft $m$ tokens from the heads, verify with one pass computing $c$ for all drafts and $p$ for the last accepted token. **Open — I26**: a token's $\mathcal{G}$ entry exists only after its final iteration (`§4.3`), so drafts within one window are mutually invisible to *global* attention until verified. Vacuous for ADR-012's independent heads, which never attend to each other; **not** vacuous for the sequential L3b fallback, which must answer it before being built. Acceptance rate per head is a first-class metric, and it is defined as **greedy acceptance against head 1** (I27): head $j$'s draft of $x_{t+j}$, made at position $t$, is *accepted* when it equals head 1's own greedy prediction of $x_{t+j}$, made at position $t+j-1$. This is the quantity H13's thresholds (ADR-013) are written against; it is not top-1 accuracy against the ground-truth token, which is logged beside it under a distinct name (`mtp_accept_h*` vs `mtp_top1_h*`). The two differ because a draft is accepted when it agrees with the verifier, not when it is right, and acceptance is the larger. The quantity that determines speculative-decode speed-up is the *expected accepted run length* per verify pass — sequential, stopping at the first rejection — which `scripts/analysis/probe_mtp_acceptance.py` reports beside the per-head marginals.
