...
Within a sequence, token $t$ at iteration $j$ needs $c^{(j)}_{s \le t}$ (or copied final states for halted $s$, `01 §4.7`). Decode is sequential per sequence; prefill tokens of one sequence proceed in parallel. Ordering is enforced at the owner; expert units are stateless.
...
|---|---|---|---|---|
| Hot | unit memory | $W$ tokens × iterations × KV bytes per owned sequence | $\beta_C$ | dense, every iteration |
| Warm | unit memory / host DRAM | horizon $H_w$ blocks | $\beta_C$ or host link | top-$K_{idx}$ blocks |
| Cold | SSD / storage cluster | unbounded | $\beta_{cold}$ | ANN hits only |

KV bytes per token per iteration $= 2 H_{kv} d_h b_{kv}$. Cold demand per generated token $= r_t \cdot (\text{cold hits}) \cdot B_{idx} \cdot 2 H_{kv} d_h b_{kv}$. S6 requires a **hit-locality model** — fraction of selected blocks per tier as a function of context length and task — produced by Tier-A runs with the index enabled (L9). P15's bandwidth claim holds iff cold hits per token × block bytes × throughput $< \beta_{cold}$, with headroom for tail behaviour.
...
3. **Scheduler**: training uses lockstep all-to-all with a barrier per iteration. A bounded-staleness asynchronous variant is a Phase-3 research item, not a prerequisite.
...
Per scenario: utilisation per unit over time; fabric bytes per token per iteration per tier, up and down; per-iteration and per-token latency distributions (p50/p99); tokens in flight; expert queue-wait distribution; cold-tier hit rate and bandwidth; placement drift; availability. Every S-experiment in `docs/03` names which of these it reads.
