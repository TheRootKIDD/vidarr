...
**Measured** (`009/010-train-step`, `small` dense, 4 × DDP, host-bounced): **67.5 k tokens/s aggregate with no gradient accumulation** = 9.6 TFLOPS per card (35 % MFU vs measured $\phi$; 12.0 incl. output head); single card 22.1 k tokens/s = 12.5 TFLOPS. The per-micro-step DDP all-reduce costs 31 % and amortises to ≈ 2 % at 0.5 M-token batches, so the accumulated rate approaches ≈ 88 k tokens/s. The dense assumption of 8 TFLOPS was conservative by 1.5×. **Recurrent rungs: only L5 is measured, and `014`'s other rungs are withdrawn.** `024-thermal-soak-bracket-move` measures DDP L5 (micro-batch 4, no accumulation) at **737.5 ms per micro-step = 44.4 k tokens/s aggregate → `screen` 6.3 h**, over 1100 steps with zero thermal-slowdown samples — the first valid un-throttled step time this rig has produced. `014` reported 2526 ms for the identical configuration; its cards were already saturated when the rung began (erratum in `014/results.md`, `04` session log 2026-09-10 very late), so **its L1–L7b rows and every `screen` hour derived from them are withdrawn**, and with them the claim that the per-micro-step all-reduce costs the recurrent rungs ≈ 8× the dense one — against `014`'s own 651 ms single-GPU L5 the overhead is ≈ 86 ms, *below* the dense rung's ≈ 250 ms (I21's premise, withdrawn). Accumulation may still be worth having; it is no longer known to be mandatory. **`018` re-measures every rung one at a time from cold with a cooldown between rungs**; until it runs, the non-L5 variant figures below are placeholders scaled from a withdrawn table and must not be used for a budget.

| Run | Cost | Dense | Variant |
|---|---|---|---|
| `screen` (1 B tok × 0.6 GFLOP) | 6 × 10¹⁷ | **4.1 h** (≈ 3.4 h accumulated, `model/` path) | **6.3 h** L5 (measured, `024`); L5d *unknown* — `014`'s 8.4 h withdrawn |
| `small` (2.5 B × 0.6 GFLOP) | 1.5 × 10¹⁸ | **10.3 h** (≈ 8.5 h accumulated) | ≈ 16 h L5 (scaled from `024`); L5d *unknown* |
