...
|---|---|---|
| CPU | AMD Threadripper PRO 3945WX — 12 cores / 24 threads, Zen 2, 8 memory channels, 128 PCIe 4.0 lanes. The controller rates DDR4-3200 (≈ 205 GB/s) but **the installed modules are DDR4-2133 and that is the binding limit** (`022`) | ~~verify memory speed actually configured~~ **done (`022`): 8/8 channels at 2133 MT/s, 1.2 V, which is the modules' rated JEDEC grade, not a fallback. The 205 GB/s figure is unreachable on this rig and must not be used in any budget** |
| Host RAM | **256 GB (251.5 GiB) DDR4-2133 registered ECC — 8 × 32 GB SK Hynix `HMA84GR7MFR4N-TF`, 2Rx4, one per channel, all 8 populated** (`020`, `021`, `022`). Was 128 GB. Theoretical 8-channel peak **136.5 GB/s**; **measured 71.0 GB/s copy, saturating at 6 threads of 12** (`025-membw`, the benchmark of record; reproduces `019`'s crude probe to 0.5 %) | holds tokenised corpus, FAISS indexes, KV "warm" tier, optimiser offload if ever needed. **The `-TF` speed code is DDR4-2133 CL15; 2933 would be `-WM`, 3200 `-XN`. Do not raise DRAM frequency — 2133 is the rating** (`022`, `04` Q16 entry). ECC live and clean: `amd64_edac` 262 144 MB, `ce_count` = `ue_count` = 0. **Done** (`025-membw`): the plateau, not a single-thread figure, is the scenario input — one thread reads 32.2 GB/s and understates the memory system by 2.2×. Past 6 threads the curve is flat to 1 %, so a host-side stage gets its full bandwidth from half the box. Now in `sim/scenarios/local_3060.yaml` citing `025` |
| GPUs | 4 × NVIDIA RTX 3060 12 GB (SM 8.6, 28 SMs) — **measured: 3 × GA104 + 1 × GA106**, 11.63 GiB usable. **Physical layout (confirmed by the user, 2026-09-10): candidate C — all four cards spaced on the board**, the vertical kit mounted under the PCIe slots and the VG4 moved to the bottom-front with the cables swapped (900 mm on the long run, 200 mm under the slots). No bottom intake fans: two frames on the floor keep the floor occupied. This is the layout `024` accepted. **2026-09-15: the GPU 0 card was swapped and the riser cable that sat under the GPU 0 slot, part-blocking that card's intake, was moved; on the bus map `GPU-4673cc7d` is still on root port `00:03.1` and `GPU-8cb4f7cc` moved `40:03.1` → `20:03.3`** (`027`); the acceptance soak on this layout is `028` | consumer Ampere; die mix is immaterial for compute (per-card $\phi$ spread 2.5 %, `003-gemm`; I11 closed) |
...
| GPU interconnect | PCIe 4.0 x16 per card (nominal 32 GB/s per direction); **no NVLink; no PCIe P2P on GeForce** — NCCL traffic bounces through host memory | **measured: gen 4 under load on all four, but only two at x16 — `GPU-88074816` and `GPU-8cb4f7cc` share one bifurcated root complex (`20:03.1` / `20:03.3`) at x8 each** (`027`, after the riser-cable move of 2026-09-15). The link width follows the *slot*, so it changes on every re-slotting: `000-env` claimed x16-on-all-four, `017` measured two cards at x8, `020` one, `021` none, `023` one, `027` two. **Immaterial for throughput** — collectives are host-bounced at 3.59 GB/s, an order of magnitude under gen4 x8 — so no budget depends on it; re-read it, never assume it; pinned host link 24–27 GB/s (`011`); **collectives $\lambda_{link}$ = 3.59 GB/s per rank, 44 µs floor** (`002-nccl`) |
...
| `small` | 12 layers, $d$ = 768, 12/4 heads (GQA), SwiGLU $d_{ff}$ = 2048, $V$ = 32 k (75.5 M non-embedding + 24.6 M embedding, ADR-025) | ≈ 0.6 GFLOP | 2048 | 2.5 B | decisive ladder steps, ≥ 2 seeds |
| `medium` | 24 layers, $d$ = 1024, 16/4 heads (GQA), $d_{ff}$ = 2816 (270 M + 33 M, ADR-025) | ≈ 2.0 GFLOP | 2048 | 7 B | confirmations, 1 seed |

`small` recurrent configuration at the same 0.6 GFLOP/token (single stream, $E$ = $F$ = 2 with skeleton $d_{ff}$ = 4$d$, fixed $r$ = 8, middle-block window $W$ = 512, budget attention keys 1024 at the 2048 context — ADR-025/027; widths are multiples of 64): the author's local default (Q6, ADR-018) is $N_e$ = 8, $k$ = 2 → $d_{ff}$ = **896**, 16.5 M expert parameters, 77 M total; the parameter-matched variant for H6 is $N_e$ = 128, $k$ = 4 → $d_{ff}$ = **448**, 132 M expert parameters, 193 M total; $N_e$ = 1 (dense recurrent block) → **1792**, 65 M total. `medium` at $r$ = 16: $N_e$ = 8, $k$ = 2 → **1920**, 144 M total; $N_e$ = 128, $k$ = 4 → **960**, 474 M total (FSDP); $N_e$ = 1 → **3776**, 108 M. Two streams (L4, optional) do **not** fit this budget at any useful $r$ — the non-expert terms alone exceed it — so L4 runs at 2× budget against a √2·$d$ single-stream comparator (ADR-026). The tile floor is $U s_{min}$ with measured $s_{min}$ = 256: 256 at $U$ = 1 (training replicas), 1024 at $U$ = 4, which 896 does not reach — L7's $g$-sweep at $U$ = 4 trades $r$ or $k$ explicitly and logs it. **Thin experts are a property of matched-FLOPs recurrence at this scale, not a bug**: the four skeleton blocks are ≈ 65 % of the non-expert budget and $r$ iterations multiply the weight passes per token. If a step needs wider experts, trade $k$ or $r$ explicitly and log it; do not silently exceed the budget. `costmodel/` is the source of truth for every width here.
...
**Measured** (`009/010-train-step`, `small` dense, 4 × DDP, host-bounced): **67.5 k tokens/s aggregate with no gradient accumulation** = 9.6 TFLOPS per card (35 % MFU vs measured $\phi$; 12.0 incl. output head); single card 22.1 k tokens/s = 12.5 TFLOPS. The per-micro-step DDP all-reduce costs 31 % and amortises to ≈ 2 % at 0.5 M-token batches, so the accumulated rate approaches ≈ 88 k tokens/s. The dense assumption of 8 TFLOPS was conservative by 1.5×. **Every rung re-measured** (`018-train-step-rungs-recheck`, `model/`, 4 × DDP, **no accumulation** — the pessimistic bound — on the candidate-C spaced layout, with thermal telemetry proving zero thermal-slowdown samples over 21.7 min of continuous load):

| rung | ms / micro-step | micro-batch / GPU | tok/s aggregate | `screen` (1 B tok) |
|---|---|---|---|---|
| L0 dense | 519 | 4 | 63.2 k | **4.4 h** |
| L1 layered MoE | 565 | 2 | 29.0 k | 9.6 h |
| L2 + parallel form | 570 | 2 | 28.7 k | 9.7 h |
| L3 + MTP (m = 4) | 628 | 2 | 26.1 k | 10.6 h |
| L5 recurrent | 737 | 4 | 44.5 k | **6.2 h** |
| L5d final-vector global | 1026 | 4 | 31.9 k | 8.7 h |
| L5 $N_e$ = 128, $k$ = 4 | 2263 | 4 | 14.5 k | **19.2 h** |
| L5 $N_e$ = 1 | 636 | 4 | 51.6 k | 5.4 h |
| L7b ($L_e$ = 2) | 815 | 4 | 40.2 k | 6.9 h |

**`014`'s timing columns are withdrawn in full** (erratum in `014/results.md`, `04` session log 2026-09-10): its cards heated progressively through the same nine-rung sequence, and the `014`/`018` ratio rises monotonically in run order — 1.34, 1.79, 2.39, 3.09, 3.43, 3.68 — including L0, which was 34 % slow. With it goes the claim that the per-micro-step all-reduce costs the recurrent rungs ≈ 8× the dense one (I21's premise, withdrawn). **`026-train-step-rungs-1gpu` closes I21**: paired with `018`, the DDP overhead is a ring all-reduce of the gradient and nothing else — 1.5× the gradient in BF16 over $\lambda_{link}$ = 3.59 GB/s (`002-nccl`) predicts six of nine rungs within 10 %, **L5 at ratio 1.02**. L5 pays **66 ms on 77.5 M parameters against L0's 65 ms on 100.1 M**: the same cost per parameter, not 8×. **Accumulation is therefore worth having for the 271 M layered rungs, where the all-reduce is ≈ 40 % of a micro-step, and close to irrelevant for the 77.5 M recurrent ones at 9–11 %** — the opposite of what this section said before. The five queued ladder rungs 106–110 cost **44.8 h** at these no-accumulation rates, less with the trainer's accumulation.
...
Noise floor: at `small`, seed-to-seed spread in eval loss is typically 0.005–0.01 nats. Thresholds in `docs/03 §1` finer than ≈ 0.02 nats are not resolvable with 2 seeds; ADR-013 turns those into non-inferiority tests and takes $\sigma$ from the measured L0 pair.
