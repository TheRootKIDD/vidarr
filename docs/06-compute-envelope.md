...
| Host RAM | **256 GB (251.5 GiB) DDR4-2133 registered ECC — 8 × 32 GB SK Hynix `HMA84GR7MFR4N-TF`, 2Rx4, one per channel, all 8 populated** (`020`, `021`, `022`). Was 128 GB. Theoretical 8-channel peak **136.5 GB/s**; **measured lower bound 70.8 GB/s copy / 29.3 GB/s triad** (`019` probe, crude) | holds tokenised corpus, FAISS indexes, KV "warm" tier, optimiser offload if ever needed. **The `-TF` speed code is DDR4-2133 CL15; 2933 would be `-WM`, 3200 `-XN`. Do not raise DRAM frequency — 2133 is the rating** (`022`, `04` Q16 entry). ECC live and clean: `amd64_edac` 262 144 MB, `ce_count` = `ue_count` = 0. A real `bench_membw.py` under its own id is owed before any of this enters `sim/scenarios/local_3060.yaml` |
...
| GPU interconnect | PCIe 4.0 x16 per card (nominal 32 GB/s per direction); **no NVLink; no PCIe P2P on GeForce** — NCCL traffic bounces through host memory | **measured: x16 gen 4 on all four under load** (`021`; `000-env` claimed it, but `017` measured two cards at x8 and `020` one — true again only since the 2026-09-10 re-slotting); pinned host link 24–27 GB/s (`011`); **collectives $\lambda_{link}$ = 3.59 GB/s per rank, 44 µs floor** (`002-nccl`) |
