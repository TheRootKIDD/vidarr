...
| GPUs | 4 × NVIDIA RTX 3060 12 GB (SM 8.6, 28 SMs) — **measured: 3 × GA104 + 1 × GA106**, 11.63 GiB usable. **Physical layout (confirmed by the user, 2026-09-10): candidate C — all four cards spaced on the board**, the vertical kit mounted under the PCIe slots and the VG4 moved to the bottom-front with the cables swapped (900 mm on the long run, 200 mm under the slots). No bottom intake fans: two frames on the floor keep the floor occupied. This is the layout `024` accepted | consumer Ampere; die mix is immaterial for compute (per-card $\phi$ spread 2.5 %, `003-gemm`; I11 closed) |
...
| GPU interconnect | PCIe 4.0 x16 per card (nominal 32 GB/s per direction); **no NVLink; no PCIe P2P on GeForce** — NCCL traffic bounces through host memory | **measured: gen 4 under load on all four, but only three at x16 — `GPU-88074816` is on an x8 root port** (`023`). The link width follows the *slot*, so it changes on every re-slotting: `000-env` claimed x16-on-all-four, `017` measured two cards at x8, `020` one, `021` none, `023` one again. **Immaterial for throughput** — collectives are host-bounced at 3.59 GB/s, an order of magnitude under gen4 x8 — so no budget depends on it; re-read it, never assume it; pinned host link 24–27 GB/s (`011`); **collectives $\lambda_{link}$ = 3.59 GB/s per rank, 44 µs floor** (`002-nccl`) |
